...
if [ "$language" = "uk" ]; then
    if [[ `service gear status | grep active` =~ "running" ]]; then
        ...
        echo -e "Зробіть бекап \e[92m$HOME/.local/share/gear/chains/gear_staging_testnet_v4/network\e[0m"
    else
    ...
    if [[ `service gear status | grep active` =~ "running" ]]; then
        ...
        echo -e "Backup \e[92m$HOME/.local/share/gear/chains/gear_staging_testnet_v4/network\e[0m"
    else
